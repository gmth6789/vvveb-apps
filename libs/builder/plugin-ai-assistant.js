document
  .querySelector("#select-actions #edit-code-btn")
  .after(
    generateElements(
      '<a id="ai-assistant-btn" href="" title="AI assistant"><i class="icon-color-wand"></i></a>'
    )[0]
  );
...
aiModal
  .querySelector(".btn-insert-content")
  .addEventListener("click", function (event) {
...
function aiAssistantSendQuery() {
  if (!chatgptOptions["key"]) {
    displayToast(
      "bg-danger",
      "Error",
      "No ChatGPT key configured! Enter a valid key in the plugin settings page."
    );
    return;
  }
