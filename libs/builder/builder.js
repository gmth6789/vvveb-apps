...
Vvveb.Builder = {
    ...
    sectionsList.forEach(function (list, i) {
      ...
      for (group in Vvveb.SectionsGroup) {
        ...
        for (i in sections) {
          ...
          if (section) {
            item =
              generateElements(`<li data-section="${group}" data-type="${sectionType}" data-search="${section.name.toLowerCase()}">
  ...
  adjustListsHeight: function () {
    let left = document.querySelectorAll(
      "#left-panel .drag-elements-sidepane > div:not(.block-preview), #left-panel .component-properties > .tab-content"
    );
    let right = document.querySelectorAll(
      "#right-panel .drag-elements-sidepane > div:not(.block-preview), #right-panel .component-properties > .tab-content"
    );
  ...
  _initDragdrop: function () {
    ...
    document.addEventListener("mousedown", function (event) {
      ...
      if (element && event.which == 1) {
        ...
        document.getElementById("component-clone")?.remove();
        document.querySelector(
          "#section-actions, #highlight-name, #select-box"
        ).style.display = "none";
...
Vvveb.CodeEditor = {
  ...
  init: function (doc) {
    document
      .querySelector("#vvveb-code-editor textarea")
      .val(Vvveb.Builder.getHtml());
...
Vvveb.Gui = {
  ...
  save: function () {
    document
      .getElementById("textarea-modal textarea")
      .val(Vvveb.Builder.getHtml());
  ...
  viewport: function () {
    ...
    document
      .querySelector(".responsive-btns .active")
...
Vvveb.StyleManager = {
  ...
  init: function (doc) {
    if (doc) {
      ...
      if (!style) {
        this.cssContainer = generateElements(
    ...
    document
      ...
      .addEventListener("mouseenter", function (e) {
        ...
        if (element) {
          ...
          document
            .querySelector(".block-preview img")
            .setAttribute("src", src).style.display = "";
        }
      });
    ...
    document
      ...
      .addEventListener("mouseleave", function (e) {
        ...
        if (element) {
          document
            .querySelector(".block-preview img")
            .setAttribute("src", "").style.display = "none";
        }
      });
    /*
  ...
  deletePage: function (element, e) {
    ...
    if (confirm(`Are you sure you want to delete "${page.file}"template?`)) {
      ...
      if (page) {
        fetch(deleteUrl, {
          method: "POST",
          body: JSON.stringify({ file: page.file }),
        })
          .then((response) => {
            if (!response.ok) {
              throw new Error(response);
        ...
        fetch(deleteUrl, {
          ...
          .then((data) => {
            ...
            if (duplicate) {
              ...
              document.querySelector(":scope > label span", element).innerHTML =
                newName;
